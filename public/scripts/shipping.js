...
const selectCart = document.querySelector('.estimator-form select');
const shippingDisp = document.querySelector('.shipping-quote .estimator-value');
...
selectCart.addEventListener('change', function() {
	const selection = this.value;
	const continent = contCodes[selection];
	if (continent === 'Europe') {
		shippingVal.value = 5;
		shippingDisp.innerText = 'Shipping to Europe - € 5';
	} else {
		shippingVal.value = 10;
		shippingDisp.innerText = 'Shipping to Rest of World - € 10';
	}
	document.querySelector('.update-shipping').classList.remove('hidden-cart');
});
...
updateBtn.addEventListener('click', () => {
	...
	grandTotal.innerText = `€ ${(parseInt(subtotal) + parseInt(shippingVal.value)).toFixed(2)}`;
});
